...
# Initialize the social capital table
social_capital <- tibble(city = character(0), neighbourhood_name = character(0), population = numeric(0),
                         ...
                         housing_need_pct_household = numeric (0), non_mover_pct_pop = numeric(0), owner_occupied_pct_household = numeric(0), 
...
for (n in c(1:nrow(neighbourhoods))) {
  ...
  social_capital[n, 6] = CTs_neighbourhood$university_education
  
  social_capital[n, 7] = CTs_neighbourhood$housing_need
  
  social_capital[n, 8] = CTs_neighbourhood$non_mover
  
  social_capital[n, 9] = CTs_neighbourhood$owner_occupied
  
  social_capital[n, 10] = CTs_neighbourhood$official_language
  
  social_capital[n, 11] = CTs_neighbourhood$citizen
  
  social_capital[n, 12] = CTs_neighbourhood$white
...
# Generate a social capital index 
...
# Export as a table
save(social_capital, file = ".Rdata")
